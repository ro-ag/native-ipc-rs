...
const REQUIRED_SEALS: libc::c_int = F_SEAL_EXEC
    | libc::F_SEAL_GROW
    | libc::F_SEAL_SHRINK
...
impl AuthenticatedChannel {
    ...
    pub fn transfer_writer(
        ...
        let result = (|| {
            self.ensure_live()?;
            let manifest = self.manifest(prepared.native, PeerAccess::ReadOnly)?;
    ...
    pub fn receive_reader(
        ...
        let result = (|| {
            self.ensure_live()?;
            if native.mapped_len != expected_len as u64 {
                return Err(LinuxError::InvalidFrame);
            }
            let manifest = self.manifest(native, PeerAccess::ReadOnly)?;
...
impl QuiescentRegion {
    ...
    pub fn new(logical_len: usize) -> Result<Self, LinuxError> {
        ...
        let raw = unsafe { libc::memfd_create(name.as_ptr(), libc::MFD_CLOEXEC | MFD_NOEXEC_SEAL) };
        if raw < 0 {
