...
the active-resource ledger, and its public spawn/bootstrap path is enabled
(2026-07-16); [`macos-supervisor-boundary.md`](macos-supervisor-boundary.md)
records the decision and its boundary.
The private
...
| `backend/macos/supervisor.rs` | module-private | private same-user supervisor boundary | Bounded authentication/spawn framing, absolute Darwin deadline, exact-Mach-audit peer facts, freshness types, and deployer-compiled policy resolution remain unreachable from the enabled public path; they await separately packaged, signed, deployer-supplied helper artifacts. No privileged service is part of the current model. |
