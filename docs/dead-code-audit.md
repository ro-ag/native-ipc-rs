...
| `backend/macos/supervisor_broker_entry.rs` | module-private | future packaged broker entry | The linear gate entry accepts only the fixed broker vector and a read-only FIFO at FD 3, sets CLOEXEC, distinguishes EOF before/after activation, rejects wrong/repeated/later data, and exposes no launch authority. A separate session-bound control plan and executable artifact remain required. |
