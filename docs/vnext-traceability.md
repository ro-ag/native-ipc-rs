...
| 4c | Authentication receipts cannot separate from native transport; receiver-capable I/O excludes coordinator-only child control | host strict Clippy; receipt mismatch, invalid facts, non-Clone and Send/!Sync assertions | no unresolved finding after two review rounds | private unreachable facade only; channel receipt adapters, executable image identity, macOS parent ACK, and native absolute-deadline I/O remain unverified |
