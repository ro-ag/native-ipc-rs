...
| R6.6 | `AtomicCapabilities` reports lock-free cross-process 32/64-bit atomics and alignments. | `capabilities` | native publish/observe | required unsupported rejects negotiation | All unverified |
...
| R7.6 | Continuous malformed/wrong-peer traffic cannot extend deadline; partial ambiguous transmission poisons. | `deadline`, transports | bounded normal operation | reject storm/partial frame timeout | All unverified |
