...
| R6.1 | Spawn uses a resolved executable, authenticates exact process, and retains race-resistant lifecycle handle. | `session::spawn`, private backend receipt/endpoint facade | matching channel+image receipt model; exact child/image identity native tests planned | receipt mismatch and non-Clone/Send/!Sync scaffold; replacement/wrong PID/image tests planned | receipt facade implemented but intentionally unreachable; all native image identity unverified |
...
| R7.7 | Native I/O/waits are nonblocking/pollable/cancellable and recompute remaining absolute time. | private `AuthenticatedControl` contract + backend transport loops | near-deadline success planned | silence/junk/fragment/exit storms planned | private deadline/poison/nonblocking facade implemented; no native adapter verified |
