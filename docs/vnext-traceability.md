...
| R6.4 | HELLO then ACCEPT/REJECT completes before transfer. | `negotiation` | compatible accept | batch-before-accept/reject/incompatible | All unverified |
...
| 4a | Finite checked limits, unforgeable target-capability representation, and one reusable monotonic absolute deadline | host strict Clippy; all-feature tests for every zero/exact/max+1 limit, effective minima, simulated native narrowing, capability rejection, and repeated-work expiry | no unresolved finding after two review rounds | portable foundation only; HELLO codec, native fact discovery, and hostile transport timing remain unverified |
