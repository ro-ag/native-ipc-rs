...
| 4b | Canonical two-sided HELLO/ACCEPT/REJECT wire and payload-bound one-shot negotiated transcript | host strict Clippy; dependency policy; golden SHA-256 vector; opaque empty/bounded payload, full-frame truncation, every reserved byte, feature/minimum/atomic/target/digest/substitution/replay/order corpus | no unresolved finding after four review rounds | private portable codec only; authenticated exact-child transport, poisoning, public ready typestate, and later transaction binding remain unverified |
